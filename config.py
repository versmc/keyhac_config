...
class KeymapConfig:
    ...
    class KeymapsImpl:
        """This class defines the implementation for each `WindowKeymap` generated by `keymap.defineWindowKeymap(...)`. 
        Methods such as `_setKeymapXXX` are accessed from KeymapConfig.setKeymap(...).
        """

        @classmethod
        def _setKeymapGlobal(cls, keymapGlobal, keymap):
            """This function set settings of `keymapGlobal`. 
            - Mode-change
            - IME-ON/OFF
            - Oneshot Slash for escaping Modifyer

            Args:
                keymapGlobal (WindowKeymap): `WindowKeymap` instance generated by `keymap.defineWindowKeymap()`.
                keymap (Keymap): `Keymap` instance. This is necessary because `keymapGlobal` has the function to change keymap modes.
            """
            ...
            keymapGlobal["U1-c"]=set_keymap_cursor    # muhenkan - c
            keymapGlobal["U1-q"]=set_keymap_celeste   # muhenkan - q
            keymapGlobal["U1-t"]=set_keymap_test      # muhenkan - t
            keymapGlobal["U1-(28)"]=set_keymap_limited_and_ime_off     # muhenkan - henkan
            #keymap_celeste["U1-(240)"]=set_keymap_limited_and_ime_on   # muhenkan - hiragana (When IME is OFF) この設定はなくてもなぜか動作する。細かい仕様は調査が必要
            keymapGlobal["U1-(242)"]=set_keymap_limited_and_ime_on     # muhenkan - hiragana (When IME is ON?)
            keymapGlobal["U1-q"]=set_keymap_celeste                    # muhenkan - q

            keymapGlobal["S-(241)"]=set_keymap_limited_and_ime_on  #Shift - katakana/hiragana/romaji
            keymapGlobal["S-(28)"]=set_keymap_limited_and_ime_off  #Shift - henkan

            for any in ("", "S-", "C-", "C-S-", "A-", "A-S-", "A-C-", "A-C-S-", "W-", "W-S-", "W-C-", "W-C-S-", "W-A-", "W-A-S-", "W-A-C-", "W-A-C-S-"):
                keymapGlobal[any+"O-Slash"]=any+"Slash"    # oneshot Slash -> Slash

            
            
        @classmethod
        def _setKeymapLimited(cls, window_keymap):
            """This function set settings of `keymapLimited`. 
            - muhenkan + {key}
            
            Args:
                window_keymap (WindowKeymap): `keymapLimited`
            """
        ...
        @classmethod
        def _setKeymapCursor(cls, window_keymap):
            """This function set settings of `keymapCursor`. 
            - some function is available without "muhenkan" key.
            
            Args:
                window_keymap (WindowKeymap): `keymapCursor`
            """
            window_keymap["A-z"]="A-Tab"
            window_keymap["A-S-z"]="A-S-Tab"
            window_keymap["RC-j"]="Enter"
        ...
        @classmethod
        def _setKeymapCeleste(cls, window_keymap):
            """This function set settings of `keymapCeleste`. 
            - No function
            
            Args:
                window_keymap (WindowKeymap): `keymapCeleste`
            """
            pass


        @classmethod
        def _setKeymapTest(cls, window_keymap,keymap):
            """テスト用キーマップモード
            ...
            if 1:         
                ...
                window_keymap["U1-q"]=abcde_delay  # 意図したとおり動くが time stamp inversion のメッセージが出る
            ...
            if 1:
                ...
                window_keymap[ "U1-Left"  ] = keymap.MoveWindowCommand( -10, 0 )
                window_keymap[ "U1-Right" ] = keymap.MoveWindowCommand( +10, 0 )
                window_keymap[ "U1-Up"    ] = keymap.MoveWindowCommand( 0, -10 )
                window_keymap[ "U1-Down"  ] = keymap.MoveWindowCommand( 0, +10 )

                # USER0-Shift-Up/Down/Left/Right : Move active window by 1 pixel unit
                window_keymap[ "U1-S-Left"  ] = keymap.MoveWindowCommand( -1, 0 )
                window_keymap[ "U1-S-Right" ] = keymap.MoveWindowCommand( +1, 0 )
                window_keymap[ "U1-S-Up"    ] = keymap.MoveWindowCommand( 0, -1 )
                window_keymap[ "U1-S-Down"  ] = keymap.MoveWindowCommand( 0, +1 )

                # USER0-Ctrl-Up/Down/Left/Right : Move active window to screen edges
                window_keymap[ "U1-C-Left"  ] = keymap.MoveWindowToMonitorEdgeCommand(0)
                window_keymap[ "U1-C-Right" ] = keymap.MoveWindowToMonitorEdgeCommand(2)
                window_keymap[ "U1-C-Up"    ] = keymap.MoveWindowToMonitorEdgeCommand(1)
                window_keymap[ "U1-C-Down"  ] = keymap.MoveWindowToMonitorEdgeCommand(3)

                # Clipboard history related
                window_keymap[ "C-S-Z"   ] = keymap.command_ClipboardList     # Open the clipboard history list
                window_keymap[ "C-S-X"   ] = keymap.command_ClipboardRotate   # Move the most recent history to tail
                window_keymap[ "C-S-A-X" ] = keymap.command_ClipboardRemove   # Remove the most recent history
                keymap.quote_mark = "> "                                    # Mark for quote pasting

                # Keyboard macro
                window_keymap[ "U1-0" ] = keymap.command_RecordToggle
                window_keymap[ "U1-1" ] = keymap.command_RecordStart
                window_keymap[ "U1-2" ] = keymap.command_RecordStop
                window_keymap[ "U1-3" ] = keymap.command_RecordPlay
                window_keymap[ "U1-4" ] = keymap.command_RecordClear
    ...
    def setKeymap(cls, keymap):
        ...
        keymapGlobal=keymap.defineWindowKeymap()   
        cls.KeymapsImpl._setKeymapGlobal(keymapGlobal,keymap)

        keymapLimited = keymap.defineWindowKeymap(check_func=cls.KeymapMode.isLimited)
        cls.KeymapsImpl._setKeymapLimited(keymapLimited)

        keymapCursor = keymap.defineWindowKeymap(check_func=cls.KeymapMode.isCursor)
        cls.KeymapsImpl._setKeymapCursor(keymapCursor)

        keymapCeleste = keymap.defineWindowKeymap(check_func=cls.KeymapMode.isCeleste)
        cls.KeymapsImpl._setKeymapCeleste(keymapCeleste)

        keymapTest = keymap.defineWindowKeymap(check_func=cls.KeymapMode.isTest)
        cls.KeymapsImpl._setKeymapTest(keymapTest,keymap)
